...
def pdf_csv_1():
    if request.method == 'POST':  
        ...
        return jsonify("static/files/pdfcsv files/"+name)
...
def pdf_img_1():
    if request.method == 'POST':  
        ...
        return jsonify("static/files/pdfimages/pdf_imgs.zip")
...
def ppt_pdf_1():
    if request.method == 'POST':  
        ...
        ppt_name = os.path.join(os.path.abspath(os.path.dirname(__file__)),app.config['UPLOAD_FOLDER'],f.filename)
        deck = powerpoint.Presentations.Open(ppt_name)
        
        output_name = os.path.join(os.path.abspath(os.path.dirname(__file__)),app.config['UPLOAD_FOLDER']+"pptpdf/",f.filename[:-4]+".pdf")
